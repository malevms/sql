/*
Optimized SQL Query for Teradata with Fixed COMBINED_TERM_CHRG

Filename Suggestion: optimized_fixed_dev_P_UPD_ENCTR_STD_GRP_PAT_CLAS.sql

Changes Made and Why:
1. Split COMBINED_TERM_CHRG into CHRG_AGG and COMBINED_TERM_CHRG:
   - Why: The original COMBINED_TERM_CHRG had SUM aggregates within the CASE statement for v_grp_cd_all, causing the Teradata error "group by and with clause may not contain aggregate function." Moved charge-related aggregates to a new CHRG_AGG CTE to compute them separately, then referenced in COMBINED_TERM_CHRG, ensuring compliance with Teradata's restrictions.

2. Maintained Differing Filters for CHRG_DTL and CHRG_DTL2:
   - Why: CHRG_DTL uses specific charge code filters (e.g., 'SURGERY', 'EMERGENCY'), while CHRG_DTL2 aggregates all charge quantities. In CHRG_AGG, applied CHRG_DTL filters within SUM(CASE ...) expressions and computed chrg_qty_cnt without filters to preserve CHRG_DTL2 logic.

3. Inlined prcs_ctrl Logic:
   - Why: Kept prcs_ctrl conditions inlined to avoid subquery overhead, as in the previous optimization.

4. Adjusted me_dt for Teradata:
   - Why: Retained ADD_MONTHS and EXTRACT for last day of month calculation, ensuring Teradata compatibility.

5. Simplified GROUP BY:
   - Why: Ensured GROUP BY in CHRG_AGG and COMBINED_TERM_CHRG only includes non-aggregated columns (enctr_id, name_space_cd, eff_from_dt), reducing hashing overhead.

6. Preserved TERM1 and TERM2 Logic:
   - Why: Integrated TERM1 ('Exclude From Reports') and TERM2 ('LEGACY CONVERSION') into COMBINED_TERM_CHRG's CASE logic, maintaining functionality.

7. Teradata-Specific Notes:
   - Ensure indexes on enctr_id, name_space_cd, eff_from_dt, eff_thru_dt, post_dt, etc.
   - Run COLLECT STATISTICS on joined columns:
     COLLECT STATISTICS ON D_ERM_IBV.ENCTR_HIST COLUMN(enctr_id, name_space_cd, eff_from_dt, eff_thru_dt);
     COLLECT STATISTICS ON D_ERM_IBV.ENCTR_CHRG_DTL COLUMN(enctr_id, name_space_cd, post_dt);
...
Note: The NOT EXISTS (TERM1) check is approximated in the CASE logic. Validate against your data to ensure correctness. If further issues arise, provide EXPLAIN plan output or table sizes for additional tuning.
*/
...
WITH me_dt AS (
    /* Compute last day of month using ADD_MONTHS and EXTRACT for Teradata */
...
CHRG_AGG AS (
    /* New CTE: Computes charge-related aggregates separately to avoid aggregates in CASE */
    SELECT
        dtl.enctr_id,
        dtl.name_space_cd,
        ...
        /* CHRG_DTL2: Aggregate all charge quantities without specific filters */
...
COMBINED_TERM_CHRG AS (
    /* Uses CHRG_AGG results to avoid aggregates in CASE */
    SELECT
        eh.enctr_id,
        eh.name_space_cd,
        me_dt.new_eff_from_dt eff_from_dt,
